---
name: ui-ux-pro-max
description: "UI/UX design intelligence for web and mobile. Searchable local database with 84 styles, 192 color palettes, 74 font pairings, 192 product types, 98 UX guidelines, 104 icon entries, 16 GSAP motion presets, and 25 chart types across 22 stacks (React, Next.js, Vue, Nuxt, Svelte, Astro, SwiftUI, React Native, Flutter, Tailwind, shadcn/ui, Jetpack Compose, Angular, Laravel, JavaFX, WPF, WinUI, Avalonia, Uno Platform, UWP, Three.js, and HTML/CSS). Includes platform-specific professional-craft rulebooks: web/desktop (anti-'AI slop' anti-patterns, pointer + keyboard state, token discipline) and native/mobile (touch targets, safe areas). Use when designing, building, or reviewing UI: pages, components, color schemes, typography, layout, accessibility, animation, or data visualization."
---
...
For the full rule list per category (all ~98 UX guidelines with rationale), read `references/quick-reference.md`.

For the professional-craft rules and canonical pre-delivery checklist, read the file matching your target platform:
- **Web / desktop** → `references/web-pro-rules.md` (anti-"AI slop" anti-patterns, color/type/spacing discipline, pointer + keyboard state, motion, forms, icons)
- **Native / mobile** → `references/pro-rules.md` (icons, touch feedback, dark-mode contrast, safe areas)
...
Resolve `<SKILL_DIR>` once, before the first call, by checking in order:
1. `${CLAUDE_PLUGIN_ROOT}/.claude/skills/ui-ux-pro-max` — installed as a plugin
2. `<project-root>/skills/ui-ux-pro-max` — vendored into the repo
3. `~/.claude/skills/ui-ux-pro-max` — user-level install

```bash
python "<SKILL_DIR>/scripts/search.py" "<query>" --domain <domain>
```

Every `${CLAUDE_PLUGIN_ROOT}/.claude/skills/ui-ux-pro-max` path below means the same thing — substitute the `<SKILL_DIR>` you resolved.
...
Start with `--design-system` for comprehensive recommendations with reasoning. **Exception:** for application UI (dashboards, editors, admin panels, internal tools), this command skews toward landing-page output — see "Known bias" under *Before Delivering UI* and query the domains directly instead:
...
Read the pro-rules file matching the **target platform** and run through its canonical Pre-Delivery Checklist. Pick one:

| Target | File | Covers |
|--------|------|--------|
| Web / desktop (Next.js, React, Vue, Svelte, Astro, Laravel, HTML/Tailwind, Electron) | `references/web-pro-rules.md` | Anti-"AI slop" anti-patterns, color/type/spacing discipline, pointer + keyboard state, motion, forms, icons |
| Native / mobile (iOS, Android, React Native, Flutter, Jetpack Compose, SwiftUI) | `references/pro-rules.md` | Icon discipline, touch feedback, light/dark contrast, safe areas, accessibility |

**For any web or desktop UI, `references/web-pro-rules.md` §0 (Anti-Patterns That Read as "AI Slop") is mandatory reading before you write the first component** — not just at delivery. It is the difference between output that looks designed and output that looks generated: decorative gradients, emoji-as-icons, glow shadows, uniform mega-radius, centered body text, competing accent hues, and decorative-only animation. Confirm zero hits before delivering.

### Known bias: `--design-system` skews toward landing pages

The composite `--design-system` command searches the `landing` and `product` domains, so for **application UI** (dashboards, editors, admin panels, internal tools) it frequently returns a marketing `PATTERN` (Hero → Features → CTA) and display-scale effects like `font-size: clamp(3rem, 10vw, 12rem); font-weight: 900`. That output is wrong for a work tool — do not apply it verbatim.

When the target is application UI, skip the composite and query the domains directly:

```bash
python "<SKILL_DIR>/scripts/search.py" "dashboard admin tool professional" --domain style
python "<SKILL_DIR>/scripts/search.py" "monochrome neutral professional" --domain color
python "<SKILL_DIR>/scripts/search.py" "dashboard admin enterprise" --domain typography
python "<SKILL_DIR>/scripts/search.py" "<stack keywords>" --stack <stack>
```

Then read `references/web-pro-rules.md` and compose the system yourself. Style `Minimalism & Swiss Style` + a monochrome-plus-single-accent palette + a single-family sans is the reliable default for professional tool UI.
